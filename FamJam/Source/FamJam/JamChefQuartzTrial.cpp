...
#include "JamChefQuartzTrial.h"

// Sets default values for this component's properties
UJamChefQuartzTrial::UJamChefQuartzTrial()
{
...
void UJamChefQuartzTrial::BeginPlay()
{
	Super::BeginPlay();
...
void UJamChefQuartzTrial::MiseEnPlace(UJamChunkLibrarian* InChunkLibrarian, UJamRecipeLibrarian* InRecipeLibrarian)
{
...
// Called every frame
void UJamChefQuartzTrial::Update(float DeltaTime)
{
	if (bShouldAttemptToReadRecipe && !bIsRunningJam)
	{
		bIsRunningJam = AttemptStartJam();
	}

	if (!bIsRunningJam) return;

	ConductCooks(DeltaTime);
}

bool UJamChefQuartzTrial::AttemptStartJam()
{
	...
	//
	JamClock->StartClock(this, JamClock);
	...
	PrepNextStep(NextStepMeasureIdx, Recipe->Steps[NextStepIdx]);

	StepIdx = NextStepIdx;
	NextStepIdx = StepIdx + 1;
	NextStepMeasureIdx = Recipe->Steps[StepIdx].MeasureLength;
	PrepNextStep(NextStepMeasureIdx, Recipe->Steps[NextStepIdx]);
...
void UJamChefQuartzTrial::FinishJam()
{
	//
	Recipe = nullptr;
...
void UJamChefQuartzTrial::ConductCooks(float DeltaTime)
{
	...
	if (MeasureIdx >= Recipe->Steps[StepIdx].NextStepPrepMeasureLength
		&&
		NextStepIdx < Recipe->Steps.Num())
	{
		// Prep next chops
	...
	if (MeasureIdx >= Recipe->Steps[StepIdx].MeasureLength)
	{
		MeasureIdx -= Recipe->Steps[StepIdx].MeasureLength;

		// Jam should have better exit then this
		if (NextStepIdx >= Recipe->Steps.Num())
		{
			FinishJam();
			return;
...
void UJamChefQuartzTrial::PrepNextStep(float StepMeasureIdx, FJamStep Step)
{
	for (FJamChop Chop : Step.Chops)
	{
		Cooks[Chop.CookName]->PrepChop(StepMeasureIdx, Chop);
	}
}
